...
- `--force`: Overwrite existing files with user modifications
- `--no-backup`: Do not create backup files when overwriting
- `--backup-dir <path>`: Directory for backup files
- `--dry-run`: Check for conflicts without writing files
...
Safely generate hooks without overwriting your customizations:

### Check for Conflicts (Dry Run)

```bash
npx bruno-api generate-hooks --dry-run
```
...
Would generate 15 files
```

### Handle User-Modified Files

When you manually edit generated files, the tool detects and protects them:
...
**Output:**
```
🚨 WARNING: 2 files have user modifications:
   - src/apis/users/get-getUser.ts
...
```bash
npx bruno-api generate-hooks --force
```

**Output:**
```
✅ Generated: src/apis/users/get-getUser.ts
   📦 Backup: src/apis/users/get-getUser.ts.backup-2024-01-15T10-30-00-000Z
...
```bash
npx bruno-api generate-hooks --force --backup-dir ./backups
```

### No Backup (Dangerous)

```bash
npx bruno-api generate-hooks --force --no-backup
```

### Auto-Generated File Marker

All generated files include a marker:

```typescript
// Auto-generated by bruno-api-typescript
// ⚠️ DO NOT EDIT THIS FILE DIRECTLY - it will be overwritten on regeneration

import { useQuery } from "@tanstack/react-query";
// ... rest of code
```

Files without this marker are considered user-modified and protected.

### Hook Name Conflict Detection

The tool warns about duplicate hook names:

```
🚨 WARNING: Duplicate hook names detected:
   - useGetUser is used in multiple files:
     • src/apis/users/get-getUser.ts
     • src/apis/admin/get-getUser.ts

This may cause import conflicts. Consider renaming the Bruno files.
```
